<div class="offcanvas offcanvas-start bg-light sidebar mb-auto" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="sidebar" aria-labelledby="offcanvasDarkLabel">
  <div class="offcanvas-body">
    <nav class="navbar-light">
      <ul class="navbar-nav">
        <li class="my-2 small fw-bold text-uppercase text-decoration-none">Filtry</li>
        <li class="my-3">
          <hr class="dropdown divider">
        </li>
        <li>
        <a class="text-muted small fw-bold text-uppercase text-decoration-none sidebar-link"
         data-bs-toggle="collapse" role="button" data-bs-target="#manufacturersGroup"  aria-controls="#manufacturersGroup" aria-expanded="false">Producenci
         <span class="bi bi-chevron-right right-icon ms-auto"></span>
        </a>

        <!--
         <div class="form-check">
            <input class="form-check-input" type="checkbox" name="check" id="manufacturer1" value="optionpr1">
            <label class="form-check-label" for="manufacturer">Producent1</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="check2" id="manufacturer2" value="optionpr2">
            <label class="form-check-label" for="manufacturer">Producent2</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="check3" id="manufacturer3" value="optionpr3">
            <label class="form-check-label" for="manufacturer">Producent3</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="check4" id="manufacturer4" value="optionpr4">
            <label class="form-check-label" for="manufacturer">Producent4</label>
        </div>
        -->


